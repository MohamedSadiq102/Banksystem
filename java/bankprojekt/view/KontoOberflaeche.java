package bankprojekt.view;

import bankprojekt.control.Controller;
...
public class KontoOberflaeche extends BorderPane {
	 ...
	 */
	public KontoOberflaeche(Controller c, Girokonto model)
	{
		...
		nummer.setText(c.getGirokontoModel().getKontonummerFormatiert());
		...
		stand.textProperty().bind(c.getGirokontoModel().kontostandProperty().asString("%.2f"));
		...
		gesperrt.selectedProperty().bindBidirectional(c.getGirokontoModel().gesperrtProperty());
		...
		adresse.textProperty().bindBidirectional(c.getGirokontoModel().getInhaber().adresseProperty());
		...
		einzahlen.setOnAction(e -> c.einzahlenClick(betrag.getText()));

		abheben = new Button("Abheben");
		aktionen.getChildren().add(abheben);
		abheben.setOnAction(e -> c.abhebenClick(betrag.getText()));
